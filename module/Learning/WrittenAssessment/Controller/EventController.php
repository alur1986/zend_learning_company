...
class EventController extends AbstractActionController
{
    ...
    public function createAction ()
    {
        ...
        if ($post = $this->post(false)) {
            try {
                ...
                return $this->redirect()->toRoute('learning/written-assessment/event/update', ['event_id' => $event['event_id']]);
            }
    ...
    //        $cookie = new SetCookie('directoryMessageSuccess', $message, time() + 30, '/');
    //        $this->getResponse()->getHeaders()->addHeader($cookie);
            if (!$request->isXmlHttpRequest()) {
                $this->flashMessenger()->addSuccessMessage($message);
            }
            return $this->redirect()->toRoute('learning/written-assessment/event/directory', ['activity_id' => $activity['activity_id']]);
        }
