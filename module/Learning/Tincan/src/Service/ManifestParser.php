...
class ManifestParser extends AbstractEntity
{
    ...
    public function readManifestFile ($manifestFileName)
    {
        ...
        $content = str_replace([ 'lang="en-US"' ], [ "" ], $content);
    ...
    /**
     * Get the manifest activities
     *
     * @return array
     */
    public function getActivities ()
    {
        $this->activities = $this->manifestContent['activities'];
    ...
    public function getItems ()
    {
        ...
        $activity = is_numeric(key(current($activities))) ? current($activities['activity']) : $activities['activity'];

        $activity['itemlocation'] = is_array($activity['launch']) ? $activity['launch']['_'] : $activity['launch'];
        if (is_array($activity['name'])) {
            $activity['name'] = $activity['name']['_'];
        }
        unset( $activity['launch']);
