...
class FileController extends AbstractActionController
{
    ...
    /**
     * Display ALL files in the scorm12 learning activity
     * Upload ONE file to the scorm12 learning activity
     */
    public function uploadAction ()
    {
        ...
        $items      = isset($activity['scorm12_items']) ? $activity['scorm12_items'] : false;
        ...
        if ($availableStorageSpace <= 0) {
            throw new Exception\UploadException('There is no available storage space in your site profile. Your allowed Storage Space is: ' . FileUtils::byteString($storageSpace) . ', you have used: ' . FileUtils::byteString($usedStorageSpace) . '.  You are unable to upload any further files at this time. Please contact your Site Adminstrator.', SiteEvent::ERROR_CODE_MAX_REACHED);
        }
        ...
        // form
        $form = $this->form('Scorm12\Form\FileUpload');
        $form->get('activity_id')->setValue($activityId);

        /* recursively copy files from DIR to DIR */
        ...
        if ($post = $this->post(false)) {
            try {
                ...
                //    	$fileManager->move($oldFilePath, $archiveFilePath, true);
                    ...
                    // this will return the name of the 'launch' file which we will use below to add the CORS JS code
                    $items = $service->retrieveItemsFromManifest($manifestFileName, $activityId, $cdnUrl, $siteUrl);

                    // from here, we can 'copy' the Scorm12 course container into the CDN site path
...
" . $contents;
                            ...
                            /*    // for Lectora we need to add the <script>...</script> into a few of the Frameset files
                                $file = $cdnPath . DIRECTORY_SEPARATOR .'a001titlemgr.html';
                                if (file_exists($file)) {
                                ...
                                $file = $cdnPath . DIRECTORY_SEPARATOR .'a001content.html';
                                if (file_exists($file)) {
...
document.domain = localLocation;" . $contents;
                                ...
                                // this is a new Articulate variation with different folder/file placement
                                $file = $cdnPath . DIRECTORY_SEPARATOR .'scormdriver/blank.html';
                                if (file_exists($file)) {
                                ...
                                $file = $cdnPath . DIRECTORY_SEPARATOR .'scormdriver/AICCComm.html';
                                if (file_exists($file)) {
                                ...
                                $file = $cdnPath . DIRECTORY_SEPARATOR .'scormcontent/index.html';
                                if (file_exists($file)) {
                                ...
                                // double check that the launch file has been upfated
                                $file = $cdnPath . DIRECTORY_SEPARATOR .'scormdriver/indexAPI.html';
                                if (file_exists($file)) {
                                    $contents = file_get_contents($file);
                                    if (strpos($contents, "localLocation.slice") == false){
                                        $contents = str_ireplace("<head>", "<head>\n<script>\n  var localLocation =  new String(window.location);\n localLocation = localLocation.slice(localLocation.indexOf('.')+1, localLocation.length);\n localLocation = localLocation.slice(0, localLocation.indexOf('/'));\n document.domain = localLocation;\n</script>\n", $contents);
                                        file_put_contents($file, $contents);
                                    }
                                }

                                // HTML5 Articulate module
...
document.domain = localLocation;" . $contents;
                                ...
                                // for some Savve Express -> end-popup.htm
                                /*$file = $cdnPath . DIRECTORY_SEPARATOR .'end-popup.htm';
                                if (file_exists($file)) {
    ...
    /**
     * Download ONE file from the scorm12 learning activity
     */
    ...
    //    $oldFileUploadPath = $oldFileUploadPath . DIRECTORY_SEPARATOR . 'archive' . DIRECTORY_SEPARATOR . $activityId;
        ...
        if (!(file_exists($fileUploadPath))) { // || file_exists($oldFileUploadPath))) {
            $this->flashMessenger()->addErrorMessage(sprintf($this->translate('A Scorm 1.2 course package needs to be uploaded. Please upload a Scorm 1.2 course package.')));
