...
namespace Report\XAPIInteractions\EventManager\Listener;
...
class RouteListener
{
    ...
    protected $routeNamePrefix = 'report/xapi-interactions*';

    private function isUnsupportedSite($siteId)
    {
        return ($siteId != 200143 && $siteId != 100001);
    }
    ...
    public function route (MvcEvent $event)
    {
        try {
            ...
            if (fnmatch($routeNamePrefix, $routeName, FNM_CASEFOLD) && (empty($identity) || $this->isUnsupportedSite($siteId))) {
                $allowed = false;
            }
    ...
    public function render (Event $event)
    {
        try {
            ...
            // show/hide interactions menu
            if (fnmatch($routeNamePrefix, $templateName, FNM_CASEFOLD) && (empty($identity) || $this->isUnsupportedSite($siteId))) {
                $allowed = false;
            }
