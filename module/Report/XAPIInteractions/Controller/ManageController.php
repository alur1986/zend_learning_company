...
class ManageController extends AbstractActionController
{
    ...
    public function activitiesAction ()
    {
        ...
        $activityIds = $session['activityId'] ?  : [];
        $request     = $this->getRequest();
        $siteId      = $this->params('site_id');

        // form
        $form = $this->form('Report\XAPIInteractions\Form\Activities');
        $form->get('activityId')
        ...
        if ($post = $this->post(false)) {
            try {
                ...
                unset($session['activityId']);
                if (isset($data['activityId']) && !empty($data['activityId'])) {
                    $session['activityId'] = $data['activityId'];
                }

                // success
                return $this->redirect()->toRoute('report/xapi-interactions/groups', ['session_id' => $sessionId]);
            }
    ...
    public function groupsAction ()
    {
        ...
        // form
        $form = $this->form('Report\XAPIInteractions\Form\Groups');
        ...
        if ($post = $this->post(false)) {
            try {
                ...
                // success
                return $this->redirect()->toRoute('report/xapi-interactions/learners', ['session_id' => $sessionId]);
            }
            catch (Exception\InvalidFormException $e) {
                // form validation exception, do nothing
            }
    ...
    public function learnersAction ()
    {
        ...
        // form
        $form = $this->form('Report\XAPIInteractions\Form\Learners');
        ...
        if ($post = $this->post(false)) {
            try {
                ...
                // success
                return $this->redirect()->toRoute('report/xapi-interactions/range', ['session_id' => $sessionId]);
            }
            catch (Exception\InvalidFormException $e) {
                // form validation exception, do nothing
            }
    ...
    public function rangeAction ()
    {
        ...
        // form
        $form = $this->form('Report\XAPIInteractions\Form\Range');
    ...
    public function reportAction ()
    {
        ...
        try {
            ...
            $templates = $this->service('Report\XAPIInteractions\Templates');

            // get the current selected template
            $templateService = $this->templateService();
            $templateId = $session['template_id'] ?  : null;

            $template = $templateId ? ($templateId == 'graphical' ? array('template_id' => 'graphical') :  $templateService->findOneTemplateById($templateId)) : $templateService->findOneTemplateBySlug('default', $siteId, 'report-xapi-interactions');
            ...
            return $this->redirect()->toRoute('report/xapi-interactions/activities', ['session_id' => $sessionId]);
        }
    }
    ...
    public function csvAction ()
    {
        ...
        try {
            ...
            $directoryPath = $options->getDirectoryPath() . DIRECTORY_SEPARATOR . 'xapi-interactions';
            Stdlib\FileUtils::makeDirectory($directoryPath);
            $storagePath = $directoryPath . DIRECTORY_SEPARATOR . 'report-' . $sessionId;
            $filename = $storagePath;

            if ($templateId === 'graphical') {
                $filename .= '.xlsx';
                // create the XLSX file
                $model = new \Savve\Xlsx\View\Model\XlsxModel();
            } else {
                $filename .= '.csv';
                // create the CSV file
